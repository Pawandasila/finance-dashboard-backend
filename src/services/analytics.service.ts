...
export const summaryAnalyticsService = async (
  ...
  return {
    ...
    percentageChange: {
      ...percentageChange,
      previousValues: {
        incomeAmount: convertToRupeesUnit(
          percentageChange.previousValues.incomeAmount
        ),
        expenseAmount: convertToRupeesUnit(
          percentageChange.previousValues.expenseAmount
        ),
        balanceAmount: convertToRupeesUnit(
...
export const chartAnalyticsService = async (
  ...
  const transaformedData = (resultData?.chartData || []).map((item: any) => ({
    date: item.date,
    income: convertToRupeesUnit(item.income),
    expenses: convertToRupeesUnit(item.expenses),
  }));
...
export const expensePieChartBreakdownService = async (
  ...
  const result = await Transaction.aggregate(pipleline);
  ...
  const transformedData = {
    totalSpent: convertToRupeesUnit(data.totalSpent),
    breakdown: data.breakdown.map((item: any) => ({
      ...item,
      value: convertToRupeesUnit(item.value),
    })),
  };
