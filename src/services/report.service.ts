...
import Transaction, { TransactionTypeEnum } from "../models/Transaction.models";
import { convertToRupeesUnit } from "../utils/format-currency";
import { format } from "date-fns";
import { reportInsightPrompt } from "../utils/prompt";
import { genAI, genAIModel } from "../configs/google-ai.config";
import { createUserContent } from "@google/genai";
import ReportModel from "../models/Report.models";
import { UpdateReportSettingType } from "../validator/report.validator";
import ReportSettingModel from "../models/Repor-setting.models";
...
export const generateReportService = async (
  ...
  const byCategory = categories.reduce(
    (acc: any, { _id, total }: any) => {
      acc[_id] = {
        amount: convertToRupeesUnit(total),
        percentage:
  ...
  return {
    period: periodLabel,
    summary: {
      income: convertToRupeesUnit(totalIncome),
      expenses: convertToRupeesUnit(totalExpenses),
      balance: convertToRupeesUnit(availableBalance),
...
async function generateInsightsAI({
  ...
  try {
    const prompt = reportInsightPrompt({
      totalIncome: convertToRupeesUnit(totalIncome),
      totalExpenses: convertToRupeesUnit(totalExpenses),
      availableBalance: convertToRupeesUnit(availableBalance),
      savingsRate: Number(savingsRate.toFixed(1)),
      categories,
      periodLabel,
    });
